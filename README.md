...
The ROS cropper node uses highlighted thermal pixels to publish `/camera/depth/cropped/image_raw` and `/thermal/cropped/image_raw`. Both outputs keep the original stream dimensions; pixels outside the selected highlighted region are blacked out. The mask follows the selected highlighted cluster shape rather than filling its bounding rectangle. Each crop unit covers `crop_unit_thermal_pixels` square thermal pixels, clusters count diagonal neighbors, `min_region_size` rejects small clusters, and `inflation_radius_thermal_pixels` expands the crop mask. The dashboard only toggles the cropper on or off at runtime; all cropper tuning values live in the `thermal_cropper` block in `config/master_params.yaml` and are passed to `thermal_cropper_node` at launch. `highlight_min_delta_from_frame_low` means the pixel must be at least that much hotter than the coldest pixel in the current frame; `highlight_max_delta_from_frame_high` means the pixel must be within that much of the hottest pixel in the current frame.
