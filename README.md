...
Full mode also provides a display-only `View > Window zoom` control from `30%` to `100%`, defaulting to `75%`. It scales the complete camera viewer, including its frame, status, and tuning panel. The setting is stored in the browser and never changes thermal alignment or ROS cropper parameters.
