...
public enum ScalarInterpretation: Equatable, Sendable {
    ...
    /// The interpretation a thermal topic should use for a given encoding.
    ///
    /// `32FC1` thermal frames are already degrees Celsius. `mono16` thermal
    /// frames from the MLX90640 bridge are conventionally centi-degrees, so the
    /// default undoes that; the value is configurable in settings because
    /// drivers disagree.
    public static func thermalDefault(for encoding: ROSImageEncoding) -> ScalarInterpretation {
        switch encoding {
        case .float32Single: return .linear(scale: 1.0, offset: 0.0, unit: .celsius)
        case .uint16Single, .mono16: return .linear(scale: 0.01, offset: 0.0, unit: .celsius)
        case .mono8: return .raw
        default: return .raw
        }
    }
