...
/// depth and thermal streams, and the controls.
///
...
struct LiveView: View {
    ...
    private var unsupportedCameraPlaceholder: some View {
        ZStack {
            ...
            VStack(spacing: 10) {
                ...
                Text("The depth and thermal streams and all diagnostics still work.")
                    .font(.caption2)
    ...
    private var calibrationLabel: String {
        guard let isCalibrated = connection.isCalibrated else { return "Unknown" }
        if !isCalibrated { return "Not calibrated" }
        guard let isTracking = connection.isVisualTracking else { return "Calibrated" }
        return isTracking ? "Calibrated" : "Calib., no vision"
    }

    private var calibrationLevel: StatusPill.Level {
        guard let isCalibrated = connection.isCalibrated else { return .neutral }
        guard isCalibrated else { return .bad }
        return (connection.isVisualTracking ?? false) ? .good : .warning
    }
    ...
    private var metricsStrip: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 10) {
                ...
                MetricTile(
                    label: "Thermal",
                    value: String(format: "%.1f fps", connection.thermalRenderFPS),
                    caption: thermalCaption,
                    isDimmed: connection.thermalFrame == nil
                )
                MetricTile(
                    label: "Odom",
    ...
    private var thermalCaption: String? {
        guard let frame = connection.thermalFrame else { return nil }
        return "\(frame.width)x\(frame.height) \(frame.encoding)"
    }
    ...
    private var sensorPanel: some View {
        VStack(spacing: 8) {
            Picker("View", selection: viewModeBinding) {
                ForEach(ImageViewMode.allCases) { mode in
            ...
            ZStack(alignment: .bottomTrailing) {
                ...
                if settings.settings.imageViewMode == .pictureInPicture, let inset = insetFrame {
                    SensorImageView(frame: inset, placeholder: "", interpolate: true)
            ...
            if settings.settings.imageViewMode == .blended {
                HStack(spacing: 8) {
                    Image(systemName: "cube.transparent").font(.caption2)
                    Slider(value: blendBinding, in: 0...1)
    ...
    private var sensorContent: some View {
        switch settings.settings.imageViewMode {
        case .depthOnly:
            SensorImageView(frame: connection.depthFrame, placeholder: depthPlaceholder)

        case .thermalOnly:
            SensorImageView(frame: connection.thermalFrame, placeholder: thermalPlaceholder, interpolate: true)

        case .blended:
            ZStack {
                SensorImageView(frame: connection.depthFrame, placeholder: depthPlaceholder)
                SensorImageView(frame: connection.thermalFrame, placeholder: "", interpolate: true)
                    .opacity(settings.settings.blendOpacity)
            }
    ...
    private var insetFrame: RenderedFrame? { connection.thermalFrame }
    ...
    private var thermalPlaceholder: String {
        connection.connectionState.isConnected
            ? "Waiting for \(RobotTopic.thermalImage.topicName)"
            : "Not connected"
    }
    ...
    private var legend: some View {
        ...
        case .thermalOnly:
            thermalLegend
        case .depthOnly, .pictureInPicture:
            depthLegend
        case .blended:
            VStack(spacing: 6) {
                depthLegend
    ...
    private var thermalLegend: some View {
        ColorLegend(
            style: settings.settings.thermalColorMap.style,
            low: connection.thermalFrame?.rangeLow ?? settings.settings.thermalColorMap.low,
            high: connection.thermalFrame?.rangeHigh ?? settings.settings.thermalColorMap.high,
            unit: connection.thermalFrame?.unit ?? .celsius,
            reversed: settings.settings.thermalColorMap.reversed
        )
    }
        ...
        VStack(spacing: 8) {
            ...
            HStack(spacing: 8) {
                ...
                Button {
                    ...
                    Label(
                        showsSensorPanel ? "Hide sensors" : "Show sensors",
    ...
    private var viewModeBinding: Binding<ImageViewMode> {
        Binding(
            get: { settings.settings.imageViewMode },
            set: { settings.settings.imageViewMode = $0 }
        )
    }

    private var blendBinding: Binding<Double> {
        Binding(
            get: { settings.settings.blendOpacity },
            set: { settings.settings.blendOpacity = $0 }
        )
    }
