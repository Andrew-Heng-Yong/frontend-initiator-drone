...
/// Colour maps, stream throttling and pose handling.
struct SettingsView: View {
    ...
    private var thermalSection: some View {
        Section {
            Picker("Colour map", selection: binding(\.thermalColorMap.style)) {
                ForEach(ColorRampStyle.allCases) { Text($0.displayName).tag($0) }
            }

            Picker("Range", selection: binding(\.thermalColorMap.rangeMode)) {
                Text("Fixed").tag(ScalarRangeMode.fixed)
                Text("Auto").tag(ScalarRangeMode.autoPerFrame)
                Text("Auto (smoothed)").tag(ScalarRangeMode.autoSmoothed)
            }

            if settings.settings.thermalColorMap.rangeMode == .fixed {
                rangeRow(
                    title: "Cold",
                    value: binding(\.thermalColorMap.low),
                    range: -20...60,
                    step: 0.5,
                    unit: "°C"
                )
                rangeRow(
                    title: "Hot",
                    value: binding(\.thermalColorMap.high),
                    range: -10...300,
                    step: 0.5,
                    unit: "°C"
                )
            ...
            ColorLegend(
                ...
                low: settings.settings.thermalColorMap.low,
                high: settings.settings.thermalColorMap.high,
                unit: .celsius,
                reversed: settings.settings.thermalColorMap.reversed
            )
        } header: {
            Text("Thermal colour map")
        } footer: {
            Text("""
            32FC1 thermal frames are taken as degrees Celsius directly. Integer frames are scaled by \
            the factor above — MLX90640 bridges commonly publish centi-degrees, so 0.01 is the usual \
            value. Check the number against a known warm object before trusting it.
            """)
        }
    }
    ...
    private var aboutSection: some View {
        Section {
            ...
            LabeledContent("Thermal topic", value: RobotTopic.thermalImage.topicName)
