...
public final class RobotConnection: ObservableObject {
    ...
    @Published public private(set) var thermalFrame: RenderedFrame?
    ...
    @Published public private(set) var thermalRenderFPS: Double = 0
    @Published public private(set) var odometryRateHz: Double = 0

    @Published public private(set) var droppedDepthFrames: Int = 0
    @Published public private(set) var droppedThermalFrames: Int = 0
    ...
    private var thermalPipeline: ImageStreamPipeline?
    ...
    private var thermalRenderRate = RateTracker(windowDuration: 2.0)
    ...
    public func apply(settings newSettings: AppSettings) {
        ...
        thermalPipeline?.colorMapSettings = newSettings.thermalColorMap
    ...
    private func makeClientConfiguration() -> RosbridgeClient.Configuration {
        RosbridgeClient.Configuration(
            ...
            throttleOverrides: [
                .depthImage: settings.imageThrottleMilliseconds,
                .thermalImage: settings.imageThrottleMilliseconds,
            ],
    ...
    private func rebuildPipelines() {
        ...
        let thermalScale = currentSettings.thermalScale
        let thermalOffset = currentSettings.thermalOffset
        let thermal = ImageStreamPipeline(
            topic: .thermalImage,
            colorMapSettings: currentSettings.thermalColorMap,
            interpretationProvider: { encoding in
                switch encoding {
                case .float32Single:
        ...
        thermal.onFrame = { [weak self] frame in
            self?.handleThermalFrame(frame)
        }
        thermal.onError = { [weak self] text in
            self?.append(log: RosbridgeLogEntry(level: .error, text: text))
        }

        depthPipeline = depth
        thermalPipeline = thermal
    }
    ...
    public func disconnect() {
        ...
        depthPipeline?.reset()
        thermalPipeline?.reset()
        sampler.reset()
        depthRenderRate.reset()
        thermalRenderRate.reset()
    ...
    private func handle(_ event: RosbridgeEvent) {
        ...
        case .image(let topic, let message):
            ...
            case .thermalImage: thermalPipeline?.submit(message)
            default: break
            }
    ...
    private func handleThermalFrame(_ frame: RenderedFrame) {
        thermalFrame = frame
        thermalRenderRate.record(at: Date().timeIntervalSince1970)
        droppedThermalFrames = thermalPipeline?.statistics.dropped ?? 0
    }
    ...
    private func refreshDerivedRates() {
        ...
        thermalRenderFPS = thermalRenderRate.rate(at: now)
        refreshTrackingStatus()
    }
    ...
    private func refreshTrackingStatus() {
        trackingStatus = RobotTrackingStatus.evaluate(
            ...
            odometryAge: sampler.age(atLocalTime: Date().timeIntervalSince1970),
