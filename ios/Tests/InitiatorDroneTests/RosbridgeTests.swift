...
final class RosbridgeTests: XCTestCase {
    ...
    func testTopicDefaultsKeepImagesThrottledAndOdometryFree() {
        ...
        XCTAssertGreaterThan(RobotTopic.thermalImage.defaultThrottleMilliseconds, 0)
    ...
    func testTopicMessageTypesMatchTheRobotContract() {
        ...
        XCTAssertEqual(RobotTopic.thermalImage.messageType, "sensor_msgs/msg/Image")
    ...
    func testThermalFixtureDecodesToCelsius() throws {
        let message = try FixtureLibrary.message(named: "thermal_mono16")
        let image = try XCTUnwrap(ROSMessageParser.image(from: message))

        guard case .scalar(let decoded) = try ROSImageDecoder.decode(
            image,
        ...
        XCTAssertEqual(decoded.unit, .celsius)
        XCTAssertEqual(Double(decoded.value(x: 0, y: 0)), 21.0, accuracy: 1e-4)
        XCTAssertEqual(Double(decoded.value(x: 2, y: 0)), 36.7, accuracy: 1e-4)
        XCTAssertEqual(Double(decoded.value(x: 2, y: 1)), 18.0, accuracy: 1e-4)
    }
    ...
    static var allTests: [(String, (RosbridgeTests) -> () throws -> Void)] {
        [
            ...
            ("testThermalFixtureDecodesToCelsius", testThermalFixtureDecodesToCelsius),
