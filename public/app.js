...
function connectRosbridge() {
  ...
  connection.textContent = 'Connecting to RGB tracking stream...';
  rosSocket.onopen = () => {
...
async function drawCameraFrame(image) {
  ...
  updateRangeLabel(width, height, 0);
  connection.textContent = `Receiving RGB tracking stream: ${imageTopic}`;
}
...
function updateRangeLabel(width, height, cropCount) {
  range.textContent = cropCount > 0
    ? `${width}x${height} | ${cropCount} tracked`
    : `${width}x${height}`;
}
